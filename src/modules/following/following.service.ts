...
import { IFollowAUser, IGetAllFollowers, IUnFollowUser } from "./following.type";
import { AlreadyExistsException, DoesNotExistsException } from "src/lib/exceptions";
...
export class FollowingService {
    ...
    async followAUser(payload: IFollowAUser) {
        try {
            ...
            if (userToFollow === userId) throw new BadRequestsException('Cannot follow self')

            const follower = await this.data.followers.findOne({ follower: userId, user: userToFollow })
            ...
            const followNotificationFactory = this.notificationFactory.create(followNotificationPayload)
            const followedNotificationFactory = this.notificationFactory.create(followedNotificationPayload)
    ...
    async unFollowUser(payload: IUnFollowUser) {
        try {
            ...
            const follower = await this.data.followers.findOne({ follower: userId, user: userToUnFollow })
    ...
    async getAllFollowers(payload: IGetAllFollowers) {
        try {
            ...
            const { data, pagination } = await this.data.followers.findAllWithPagination(filterQuery)
