...
import { AlreadyExistsException, BadRequestsException, DoesNotExistsException, TooManyRequestsException } from 'src/lib/exceptions';
import { OptionalQuery } from 'src/core/types/database';
import { User } from '../user/entities/user.entity';
import { compareHash, hash, isEmpty, randomFixedInteger, secondsToDhms } from 'src/lib/utils';
import { UserFactoryService } from '../user/user-factory.service';
import { INCOMPLETE_AUTH_TOKEN_VALID_TIME, JWT_USER_PAYLOAD_TYPE, RESET_PASSWORD_EXPIRY, RedisPrefix } from 'src/lib/constants';
...
export class AuthService {
  ...
  async login(payload: ILogin) {
    try {
      ...
      if (username) {
        const usernameExists = await this.data.users.findOne({ username })
        if (!usernameExists) throw new DoesNotExistsException('Invalid username or password')

          const validPassword: boolean = await compareHash(password, usernameExists?.password)
          if (!validPassword) throw new BadRequestException('Invalid username or password')
          ...
          await this.data.users.update({ _id: usernameExists._id }, { $set: { lastLoginDate: new Date() }})
      ...
      if (email) {
        const emailExists = await this.data.users.findOne({ email })
        if (!emailExists) throw new DoesNotExistsException('Invalid email or password')

          const validPassword: boolean = await compareHash(password, emailExists?.password)
          ...
          await this.data.users.update({ _id: emailExists._id }, { $set: { lastLoginDate: new Date() }})
  ...
  async authUser(userId: string) {
    try {
      const user = await this.data.users.findOne({ _id: userId }, null, { select: ['-password'] })
      return {
        status: HttpStatus.OK,
