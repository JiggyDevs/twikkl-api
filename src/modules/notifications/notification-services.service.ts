...
import { IClickAllNotifications, IClickedNotification, ICreateNotification, IGetNotifications } from "./notification.type";
...
export class NotificationService {
  ...
  async getNotifications(payload: IGetNotifications) {
    try {
      ...
      const { data, pagination } = await this.data.notification.findAllWithPagination(filterQuery)
  ...
  async clickOnNotification(payload: IClickedNotification) {
    try {
      ...
      const notification = await this.data.notification.findOne({ _id: notificationId })
      if (!notification) throw new DoesNotExistsException('Notification not found')

      await this.data.notification.update({ _id: notification._id }, { clicked: true })
  ...
  async markAllNotificationsAsRead(payload: IClickAllNotifications) {
    try {
      ...
      for (let i = 0; i < notificationIds.length; i++) {
        const notification = await this.data.notification.findOne({ _id: notificationIds[i] })
