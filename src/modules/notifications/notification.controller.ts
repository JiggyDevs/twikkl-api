...
import { FindByNotificationId, IClickAllNotifications, IClickedNotification, IGetNotifications } from "./notification.type";
...
export class NotificationController {
    ...
    async getAllNotifications(@Res() res: Response, @Query() query: any, @Param() param: FindByUserId) {
        const { userId } = param
        query = { user: userId }
    ...
    @UseGuards(StrictAuthGuard)
    async clickOnNotification(@Res() res: Response, @Param() param: FindByNotificationId) {
    ...
    @Patch('/')
    @UseGuards(StrictAuthGuard)
    async markAllAsRead(@Res() res: Response, @Body() body: MarkAllNotificationsDto) {
