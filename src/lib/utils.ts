...
export const isVerified = (emailVerified: boolean, phoneVerified: boolean): boolean => {
...
export const basicWordFinder = (_word: string, _word1: string, _word2: string): boolean => {
  const lowerCaseWord = _word.toLowerCase();
  // if either any of them is not found throw an error
  if (lowerCaseWord.indexOf(_word1.toLowerCase()) === -1 || lowerCaseWord.indexOf(_word2.toLowerCase()) === -1) return false;
  return true;
};

export const wordFinder = (_word: string, _word1: string, _word2: string): boolean => {
